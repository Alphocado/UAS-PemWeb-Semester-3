<?php
// app/Views/resep.php
// Diharapkan $recipes = array dari ResepModel, $q = query
?>
  ...
  <?php else: ?>
    <div class="row row-cols-1 row-cols-md-3 g-4">
      <?php foreach ($recipes as $r): 
        // beberapa field: id, makanan_id, bahan, langkah, gambar, kategori
        $title = $r['makanan_id'] && isset($r['nama_makanan']) ? $r['nama_makanan'] : ($r['kategori'] ?? 'Resep');
        $excerpt = substr(strip_tags($r['bahan'] ?? $r['langkah'] ?? ''), 0, 120);
        $img = !empty($r['gambar']) ? base_url('assets/img/resep/'.$r['gambar']) : base_url('assets/img/hero/pembuka.jpeg');
      ?>
      <div class="col">
        <div class="card h-100 shadow-sm">
          <img src="<?= esc($img) ?>" class="card-img-top" alt="<?= esc($title) ?>" style="object-fit:cover; height:180px;">
